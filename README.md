...
开发流程参考 Anthropic 的 SDLC Playbook：意图和验收 → 计划 → 实现与审查 → CI → 版本发布 → 问题回流。项目保留自身记录与 GitHub 配置；通用宿主流程工具独立存放，不是安装、测试或运行本项目的依赖。发布包独立解压复验，CI 或发布失败转为去重的问题单。[工作流说明](docs/WORKFLOW.md)区分自动检查、宿主自查与真实 CU 验收。Codex Hooks 是可选辅助，自动事件触发仍未留证，[历史状态](docs/changes/2026-09-21-hooks-activation.md)不替代云端 CI。
